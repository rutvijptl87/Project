...
const AddressListPage = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      const r = await api.get('/addresses', { params });
      ...
      <div className="flex flex-wrap items-center justify-between px-6 py-4 bg-white border-b border-gray-100 sticky top-16 z-10 gap-4">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-2">
            ...
            <Link to="/addresses/new" className="flex items-center gap-1 px-4 py-1.5 text-[13px] font-medium text-white bg-gray-900 hover:bg-black rounded-md shadow-sm ml-1 transition-colors">
        ...
        <div className="flex-1 flex flex-col p-6 overflow-y-auto bg-white">
          ...
          <div className="flex flex-col mb-4">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <input type="text" placeholder="Address Type" className="w-32 text-[13px] bg-gray-50 border-0 rounded-full px-3 py-1.5 text-gray-800 placeholder-gray-400 focus:bg-white focus:ring-1 focus:ring-blue-500 transition-colors" />
          ...
          <div className="bg-white overflow-x-auto mt-2 border border-gray-100 rounded-lg">
            <table className="w-full text-left text-[13px] whitespace-nowrap min-w-[600px]">
              <thead className="bg-[#111827] text-white">
                <tr>
                  <th className="px-4 py-3 w-8"><input type="checkbox" className="rounded-sm border-gray-300" checked={sortedAddresses.length > 0 && selectedItems.length === sortedAddresses.length} onChange={(e) => setSelectedItems(e.target.checked ? sortedAddresses.map(i => i.id) : [])} /></th>
                  ...
                  <th className="px-3 py-2 font-medium cursor-pointer hover:bg-gray-800 transition-colors" onClick={() => toggleSort('address_type')}>
                    <div className="flex items-center gap-1">Address Type {sortBy === 'address_type' ? <ArrowUpDown size={12} className="text-gray-300"/> : <ArrowUpDown size={12} className="opacity-20"/>}</div>
                  </th>
              ...
              <tbody className="divide-y divide-gray-50">
                {loading && page === 1 ? (
                  ...
                  <tr>
                    <td colSpan="6" className="px-4 py-8 text-center text-gray-400 text-sm">No addresses found</td>
                  </tr>
                ) : (
